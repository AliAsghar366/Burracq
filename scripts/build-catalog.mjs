...
// Real prices scraped from the original site (scripts/fetch-prices.mjs).
// Map of product URL -> price. Products without a scraped price fall back to
// the old placeholder formula below.
let realPrices = {};
try {
...
const products = rawProducts.map((p, i) => {
  ...
  let compareAtPrice;
  if (real != null && real > 0) {
    // Actual (compare-at) price: real price + 70%.
    compareAtPrice = Number((real * 1.7).toFixed(2));
    // Discounted price: 80% of the actual price.
    price = Number((compareAtPrice * 0.8).toFixed(2));
  } else {
    // Placeholder fallback for products whose real price wasn't scraped.
    price = Number(((i % 8) * 1.5 + 7.99).toFixed(2));
    compareAtPrice = Number((price * 1.8).toFixed(2));
  }
  const desc =
...
//
// Prices: \`compareAtPrice\` is the actual price (real price + 70%), \`price\` is
// the discounted price (80% of the actual). Cart totals use \`price\`.
// Products missing from crawled-prices.json fall back to placeholder prices.
...
const withRealCount = rawProducts.filter((p) => realPrices[p.url] != null).length;
console.log(
  `wrote src/data/catalog.ts: ${products.length} products, ${enrichedCategories.length} categories ` +
    `(${withRealCount}/${rawProducts.length} entries with real prices)`
);